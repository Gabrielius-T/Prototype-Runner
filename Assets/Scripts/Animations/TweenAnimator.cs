...
{
    [SerializeField]
    public List<TweenData> data;

    [SerializeField] 
    public List<UnityEvent> customBeginEvents;
    
    [SerializeField] 
    public List<UnityEvent> customCompleteEvents;

    const double TOLERANCE = 0.001f;

    Coroutine completeEventCoroutine;
    Coroutine beginEventCoroutine;
    bool performEvents;
    int currentTween;
    ...
    public void PerformTween(int _index)
    {
        ...
        if (performEvents)
        {
            if (_index < customBeginEvents.Count && data[_index].performBeginEvent)
            {
                beginEventCoroutine = StartCoroutine(PerformBeginEventIn(_index));
            }

            if (data[_index].performCompleteEvent)
            {
                completeEventCoroutine = StartCoroutine(PerformCompleteEventIn(_index));
            }
        }
    ...
    {
        LeanTween.cancel(gameObject);
        if (completeEventCoroutine != null)
        {
            StopCoroutine(completeEventCoroutine);
        }
    ...
    IEnumerator PerformCompleteEventIn(int _index)
    {
        yield return new WaitForSeconds(data[_index].duration + data[_index].delay);
        customCompleteEvents[_index]?.Invoke();
    }
    
    IEnumerator PerformBeginEventIn(int _index)
    {
        yield return new WaitForSeconds(data[_index].delay);
